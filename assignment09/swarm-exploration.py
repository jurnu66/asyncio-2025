...
if __name__ == "__main__":
    asyncio.run(main())
    # Calculate and print the time (in seconds) when the target was found
    if found_by:
        # Since all agents stop when the target is found, get the minimum step
        min_step = min(found_by.values())
        seconds = min_step * 0.01
        print(f"Target found at {seconds:.2f} seconds.")